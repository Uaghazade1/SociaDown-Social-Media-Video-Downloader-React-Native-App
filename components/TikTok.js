...
function TikTok() {
  ...
  const handleSubmit = async () => {
    ...
    const url = `https://tiktok-download-without-watermark.p.rapidapi.com/analysis?url=${encodeURIComponent(inputUrlRef)}&hd=0`;
    const options = {
      method: 'GET',
      headers: {
    ...
    try {
      ...
      if (result.code === 0 && result.data && result.data.play) {
        const playUrl = result.data.play;
        console.log('Navigating to VideoScreen with URL:', playUrl);
        navigation.navigate('VideoScreen', { videoUrl: playUrl, source: 'TikTok' });
      } else {
        Alert.alert('Error', 'TikTok URL not found.');
        console.log("TikTok URL not found");
      }
    } catch (error) {
